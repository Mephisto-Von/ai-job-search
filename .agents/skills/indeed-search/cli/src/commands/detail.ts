...
function normalizeId(input: string): string | null {
  ...
  // Accept an Indeed viewjob URL
  const urlMatch = input.match(/[?&]jk=([^&]+)/) || input.match(/viewjob\?[^j]*jk=([^&]+)/)
  if (urlMatch) return urlMatch[1]
  
  // Accept a URL with job ID in path
  const pathMatch = input.match(/\/viewjob\?.*?id=(\d+)/)
...
export async function runDetail(opts: DetailOpts): Promise<number> {
  ...
  try {
    const html = await htmlFetch(`${DETAIL_URL}?jk=${id}`)
    if (!html) {
