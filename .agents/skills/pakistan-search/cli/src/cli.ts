#!/usr/bin/env bun
// Self-contained CLI for searching jobs on Pakistani job portals.
...
const HELP = `pakistan-cli — search jobs on Pakistani job portals (Rozee.pk, Mustakbil, Indeed Pakistan)
...
SEARCH FLAGS
  ...
  --location, -l <text>   Location to search. REQUIRED. e.g. "Karachi", "Lahore",
                          "Islamabad", "Rawalpindi".
  --jobage <days>         Posted within N days: 1, 7, 14, 30. Default: all.
  --page <n>              1-indexed page. Default 1.
  --limit, -n <n>         Cap results emitted (client-side).
  --salary <amount>       Minimum salary filter (PKR).
...
EXAMPLES
  bun run src/cli.ts search -q "software engineer" -l "Karachi" --jobage 30 --format table
  bun run src/cli.ts search -q "marketing manager" -l "Lahore" --format table
  bun run src/cli.ts search -q "remote" -l "Pakistan" --format table
  bun run src/cli.ts company "Jazz" --location "Karachi" --format table
  bun run src/cli.ts detail rozee-12345 --format plain

Personal use only — uses Pakistani job portals' public pages; keep volume low.
`
  ...
  if (cmd === "search") {
    ...
    if (!location) {
      process.stderr.write(
        JSON.stringify({
          error: 'the --location/-l flag is required (e.g. -l "Karachi", -l "Lahore", -l "Islamabad")',
          code: "NO_LOCATION",
        }) + "\n",
      )
      return 1
